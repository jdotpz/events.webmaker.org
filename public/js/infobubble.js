...
InfoBubble.prototype.buildDom_ = function() {
  ...
  this.contentContainer_.classList.add('info-bubble-container');  // added by SNG
...
InfoBubble.prototype.updateArrowStyle_ = function() {
  ...
  // this fixed some random drawing issues, it was too high by one pixel sometimes (SNG)
  inner.style['top'] = '-1px';
  outer.style['top'] = '1px';
};
...
InfoBubble.prototype.panToView = function() {
  ...
  var iwWidth = this.bubble_.offsetWidth;
  var iwHeight = this.bubble_.offsetHeight;
  var halfWidth = (iwWidth/2);
  var padX = 10;
  var padY = 10;
  var pixPosition = this.getProjection().fromLatLngToContainerPixel(this.getPosition());

  var xOffset = 0, yOffset = 0;

  if ((pixPosition.x - halfWidth) < padX) {
    var oldX = pixPosition.x - halfWidth;
    var newX = padX;

    xOffset = newX - oldX;
  } else if ((pixPosition.x + halfWidth) > (mapWidth - padX)) {
    var oldX = pixPosition.x + halfWidth;
    var newX = mapWidth - padX;

    xOffset = newX - oldX;
  }

  var bottomHeight = (this.getAnchorHeight_() + this.getArrowSize_());
  var positionY = pixPosition.y - bottomHeight;
  if ((positionY - iwHeight) < padY) {
    var oldY = positionY - iwHeight;
    var newY = padY;

    yOffset = newY - oldY;
  } else if (pixPosition.y > (mapHeight - padY)) {
    var oldY = pixPosition.y;
    var newY = (mapHeight - padY);

    yOffset = newY - oldY;
  }

  if (!(xOffset === 0 && yOffset === 0)) {
    // changes the center point of the map by x and y
